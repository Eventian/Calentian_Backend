...
app.get('/event-api/api/locations', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const [results] = await connection.execute(query, [req.user.calentian_entries_id]);
...
try {
  const connection = await initDB();
  const [results] = await connection.execute(query, [eventId, req.user.calentian_entries_id]);
  await connection.end();
  ...
  event.customer_emails = event.customer_emails ? event.customer_emails.split(',') : [];
  res.json(event);
} catch (err) {
...
app.post('/event-api/api/events', authenticateToken, async (req, res) => {
  const { kunden_id, location_id, datum, veranstaltungsart, status, anzahl_personen_gesamt, anzahl_kinder, event_name } = req.body;
  ...
  try {
    ...
    if (customerRows.length > 0) {
      customerName = customerRows[0].firma && customerRows[0].firma.trim() !== ''
        ? customerRows[0].firma
        : customerRows[0].vorname;
    }

    // Fallback für event_name, falls nicht gesetzt
    const finalEventName = event_name && event_name.trim() !== ''
      ? event_name
    ...
    const [result] = await connection.execute(query, [
      ...
      location_id, 
      datum, 
      veranstaltungsart, // Hier wird die ID der Veranstaltungsart übergeben
      status, 
    ...
    await connection.end();
    res.status(201).json({ message: '✅ Event erfolgreich erstellt', eventId: result.insertId });
  } catch (err) {
...
app.post('/event-api/api/customers', authenticateToken, async (req, res) => { 
  ...
  const { vorname, nachname, firma, strasse, plz, stadt, telefonnummer, email, calentian_entries_id } = req.body;

  if (!vorname || !nachname || !email) {
    return res.status(400).json({ message: 'Vorname, Nachname und Email sind Pflichtfelder!' });
  }
  ...
  try {
    ...
    const [result] = await connection.execute(
      `INSERT INTO calentian_kundendaten (vorname, nachname, firma, strasse, plz, stadt, telefonnummer, calentian_entries_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [vorname, nachname, firma || null, strasse, plz, stadt, telefonnummer || null, calentian_entries_id]
    );
...
app.get('/event-api/api/customers/email-exists', authenticateToken, async (req, res) => {
...
app.get('/event-api/api/emails/:eventId', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ message: 'Fehler beim Laden der Nachrichten', err });
  }
});

//🔒 API-Route: Status der Nachrichten ändern
app.post('/event-api/api/emails/update-status', authenticateToken, async (req, res) => {
  ...
  if (!email_id || status === undefined) {
    return res.status(400).json({ message: 'Email-ID und Status müssen angegeben werden.' });
  }

  // Optional: Erlaube nur die Werte 1 (ungelesen) und 2 (gelesen)
  if (![1, 2].includes(status)) {
    return res.status(400).json({ message: 'Ungültiger Statuswert! Erlaubt sind 1 (ungelesen) und 2 (gelesen).' });
  }

  const query = 'UPDATE calentian_kunden_emails SET calentian_email_status_id = ? WHERE id = ?';
  ...
  try {
    ...
    res.status(200).json({ message: '✅ Email-Status erfolgreich aktualisiert', affectedRows: result.affectedRows });
  } catch (err) {
    console.error('❌ Fehler beim Aktualisieren des Email-Status:', err);
    res.status(500).json({ message: 'Fehler beim Aktualisieren des Email-Status' });
  }
});
...
app.get('/event-api/api/user-locations', authenticateToken, async (req, res) => {
  const entryId = req.user.calentian_entries_id;
  if (!entryId) {
    return res.status(400).json({ message: 'calentian_entries_id fehlt im Token.' });
  }
  ...
  try {
    ...
    res.status(500).json({ message: 'Fehler beim Abrufen der User-Locations' });
  }
});
...
app.put('/event-api/api/events/:id', authenticateToken, async (req, res) => {
  const eventId = req.params.id;
  const { veranstaltungsart, datum, status, location_id, event_name, calentian_kundendaten_id, anzahl_personen_gesamt, anzahl_kinder } = req.body;
  ...
  const query = `UPDATE calentian_event_entries SET ${updates.join(', ')} WHERE id = ?`;
  values.push(eventId);
  ...
  try {
    ...
    const [result] = await connection.execute(query, values);
    await connection.end();
    res.json({ message: '✅ Event erfolgreich aktualisiert', affectedRows: result.affectedRows });
  } catch (err) {
...
app.get('/event-api/api/veranstaltungsart', authenticateToken, async (req, res) => {
  try {
    const connection = await initDB();
    const [results] = await connection.execute(
